...
function assertContains(object: Record<any, unknown>, pairs: Record<any, unknown>) {
...
describe('bullmq', () => {
  ...
  describe('Queue', () => {
    ...
    it('should create a queue span and a job span for add', async () => {
      ...
      const queueAddSpan = spans.find(span => span.name === 'queueName.jobName Queue.add');
      ...
      const jobAddSpan = spans.find(span => span.name === 'queueName.jobName Job.addJob');
    ...
    it('should create a queue span and many job spans for addBulk', async () => {
      ...
      const queueAddBulkSpan = spans.find(span => span.name === ('queueName Queue.addBulk'));
      ...
      assertDoesNotContain(queueAddBulkSpan?.attributes!, ['messaging.bullmq.job.name'])


      const jobAddSpan1 = spans.find(span => span.name === 'queueName.jobName1 Job.addJob');
      const jobAddSpan2 = spans.find(span => span.name === 'queueName.jobName2 Job.addJob');
  ...
  describe('FlowProducer', () => {
    ...
    it('should create a queue span and a job span for add', async () => {
      ...
      // TODO: rename to `messaging.message.id`
      assert.strictEqual(typeof jobAddSpan?.attributes!['message.id'], 'string')
    ...
    it('should create a queue span and many job spans for add with children', async () => {
      ...
      assertContains(jobAddSpan?.attributes!, {
        'messaging.destination': 'queueName',
        'messaging.bullmq.job.name': 'jobName',
        'messaging.bullmq.job.parentOpts.waitChildrenKey': 'bull:queueName:waiting-children',
      })
      assert.strictEqual(typeof jobAddSpan?.attributes!['message.id'], 'string')
      assert.notStrictEqual(jobAddSpan?.attributes!['message.id'], 'unknown')
      ...
      assert.strictEqual(typeof childJobAddSpan?.attributes!['message.id'], 'string')
      assert.notStrictEqual(childJobAddSpan?.attributes!['message.id'], 'unknown')
    ...
    it('should create a queue span and many job spans for addBulk', async () => {
      ...
      assertDoesNotContain(flowProducerAddBulkSpan?.attributes!,
        ['messaging.destination', 'messaging.bullmq.job.name'])
  ...
  describe('Worker', () => {
    it('should not generate any spans when disabled', async () => {
      ...
      instrumentation.disable();
      const w = new Worker('disabled', async () => {
        processorDone()
    ...
    it('should create a span for the worker run', async () => {
      ...
      const workerRunSpan = spans.find(span => span.name === 'queueName Worker.run');
    ...
    it('should only create a span for the worker run when it runs', async () => {
      const w = new Worker('queueName', async () => undefined, {connection, autorun: false})
    ...
    it('should create a span for the job attempt', async () => {
      ...
      const jobAddSpan = spans.find(span => span.name === 'queueName.testJob Job.addJob');
      assert.notStrictEqual(jobAddSpan, undefined);

      const workerJobSpan = spans.find(span => span.name === 'queueName.testJob Worker.queueName #1');
      ...
      assert.strictEqual(
        typeof workerJobSpan?.attributes!['messaging.bullmq.job.timestamp'], 'number'
      )
      assert.strictEqual(
        typeof workerJobSpan?.attributes!['messaging.bullmq.job.processedOn'], 'number'
      )
      assert.ok(
    ...
    it('should create spans for each job attempt', async () => {
      ...
      await q.add('testJob', {started: new Date().toTimeString()}, {attempts: 3});
      ...
      const firstJobSpan = spans.find(span => span.name.includes('Worker.worker #1'));
      assert.notStrictEqual(firstJobSpan, undefined);
      assertContains(firstJobSpan?.attributes!, {
        'messaging.bullmq.job.attempts': 1,
      })
      assertDoesNotContain(firstJobSpan?.attributes!, ['messaging.bullmq.job.failedReason'])
      assert.strictEqual(firstJobSpan?.events.length, 1);

      const secondJobSpan = spans.find(span => span.name.includes('Worker.worker #2'));
      ...
      const thirdJobSpan = spans.find(span => span.name.includes('Worker.worker #3'));
